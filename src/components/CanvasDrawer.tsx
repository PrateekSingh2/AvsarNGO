...
interface CanvasDrawerProps { bgImage?: string; bgText?: string; onDrawStart?: () => void; onClear?: () => void; onSave?: (image: string) => void; fullScreen?: boolean; }
...
export function CanvasDrawer({ bgImage, bgText, onDrawStart, onClear, onSave, fullScreen = false }: CanvasDrawerProps) {
  ...
  const initCanvas = useCallback(() => { const canvas = canvasRef.current; if (!canvas) return; const ctx = canvas.getContext('2d'); if (!ctx) return; const old = canvas.toDataURL?.(); const rect = canvas.getBoundingClientRect(); canvas.width = rect.width * 2; canvas.height = rect.height * 2; ctx.scale(2, 2); ctx.lineCap = 'round'; ctx.lineJoin = 'round'; if (old) restore(old); }, []);
  ...
  const save = () => { const image = canvasRef.current?.toDataURL('image/png') || ''; if (!image) return; onSave?.(image); const a = document.createElement('a'); a.download = 'avsar-art.png'; a.href = image; a.click(); audioManager.playSound('reward'); };
  ...
  const shape = (kind: 'circle'|'square') => { const ctx = canvasRef.current?.getContext('2d'); if (!ctx) return; snapshot(); ctx.strokeStyle = color; ctx.lineWidth = brush/2; if (kind === 'circle') ctx.stroke(new Path2D('M120 80 a60 60 0 1 0 1 0')); else ctx.strokeRect(80, 60, 110, 110); };
